...
function loadAndPlotData() {
  ...
  // Add vertical lines for selected prediction model
  let verticalLines = [];
  const currentYear = String(new Date().getFullYear());
  ...
  dailyChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y2: { position: "right", title: { display: true, text: "Rain Total (in)" }, grid: { drawOnChartArea: false }, beginAtZero: true },
        y3: { position: "right", title: { display: true, text: "Sunspots Daily Total" }, grid: { drawOnChartArea: false }, beginAtZero: true }
      }
    }
  });
